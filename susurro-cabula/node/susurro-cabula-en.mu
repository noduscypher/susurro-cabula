...
audio   = the sound
sync    = the time
control = each machine's own knobs (they move only it)
...
>s-1 · note
freq/reso only touch the s-1 sound.
they do not filter the audio coming in via mix in.

`F222freq opens the window. reso makes the stone sing.`f
...
>if it hisses in the phones
loose cable · 3.5/rca adapter · ts used as audio by mistake
high gain · usb adding noise · open channel with no source
test: p-6 on battery only, no usb, no sync.

-

>record a clean sample
p-6 isolated: battery only · no mix out · no sync · no usb
sampling → empty pad → maracas at 20–30 cm
sampling (record) → ~2s → stop
listen → delete if it's no good

-

>banks
a/e · b/f · c/g · d/h     1× / 2×
8 banks × 6 pads = 48 samples
